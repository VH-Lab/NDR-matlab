function [d,si,h]=abfload3(fn,varargin)
% ** function [d,si,h]=abfload3(fn,varargin)
    ...
    if doLoadData && isfield(h,'DACEpoch') && ~isempty(h.DACEpoch)
        ...
        for dac_num=1:num_dacs
            for i=1:nSweeps
                ...
                pointStart = 0;
                for epochIdx = 1:numel(h.DACEpoch(dac_num).nEpochType)
                    level = h.DACEpoch(dac_num).fEpochInitLevel(epochIdx) + (sweeps(i)-1) * h.DACEpoch(dac_num).fEpochLevelInc(epochIdx);
                    duration = h.DACEpoch(dac_num).lEpochInitDuration(epochIdx) + (sweeps(i)-1) * h.DACEpoch(dac_num).lEpochDurationInc(epochIdx);

                    if pointStart + duration > h.sweepLengthInPts
                        duration = h.sweepLengthInPts - pointStart;
                    end
                    if (duration>0)
                        waveform(pointStart+1:pointStart+duration) = level;
                    end
