...
% ndr.format.stereoseq.readGEF - read a Stereo-seq GEF into flat records
%
%   [X, Y, GENEINDEX, COUNT, GENEID, GENENAME, META] = ...
%       ndr.format.stereoseq.READGEF(GEFPATH)
%   [...] = ndr.format.stereoseq.READGEF(..., 'probeOnly', true)
%
%   Reads a BGI/MGI Stereo-seq .gef (an HDF5 file) and returns one record
%   per (pixel, gene) pair: X and Y in source units, GENEINDEX a
%   ZERO-BASED row of the returned gene list, COUNT saturating at
%   countCeiling.
%
%   This is the raw-format layer. It reads the vendor's file and returns
%   arrays; it creates no documents and knows nothing about NDI. The four
%   arrays are exactly what ndi.fun.doc.gene.makePyramid takes as its
...
if isempty(root)
    error('NDR:stereoseq:readGEF:noGeneExpressionPair', ...
...
error('NDR:stereoseq:readGEF:noSuchField', 'None of {%s} among {%s}.', ...
