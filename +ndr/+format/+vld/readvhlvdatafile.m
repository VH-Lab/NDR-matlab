function [T,D,tot_sam,tot_time] = readvhlvdatafile(myfilename, headerstruct, channelnums, t0, t1, varargin)
% READVHLVDATAFILE - Read LabView data from the VH Lab (VHLV) format
%
%  [T,D,TOTAL_SAMPLES,TOTAL_TIME] = ndr.format.vld.readvhlvdatafile(FILENAME, HEADERSTRUCT, CHANNELNUMS, T0, T1)
%
%  This function reads data from the multichannel VHLab LabView binary
%  data file format. FILENAME is the name of the file to open (extension '.vld'),
%  HEADERSTRUCT is the header structure as returned from
%  ndr.format.vld.readvhlvheaderfile, CHANNELNUMS are the channel numbers to read, where
%  1 is the first channel in the list that was acquired in LabView, 2 is
...
%
%  Example:
%     myheader = ndr.format.vld.readvhlvheaderfile('vhlvanaloginput.vlh');
%
%     % read from 0 to 5 seconds on the first channel acquired
%     [T,D] = ndr.format.vld.readvhlvdatafile('vhlvanaloginput.vld',myheader,1,0,5);
%     figure;
%     plot(T,D);
%     xlabel('Time(s)');
...
UseWaitBar = 0;
WaitBarMessage = 'Reading data from LabView file';
...
if multiplexed,
	[D, T] = readvhlvdatafile_multiplexed(fid, headerstruct, channelnums, t0, t1, precision, output_precision, unit_size, maxint,tot_sam,...
		UseWaitBar,WaitBarMessage);
	fclose(fid);
	return;
end;
...
function [data,t] = readvhlvdatafile_multiplexed(fid, headerstruct, channels, t0, t1, precision, output_precision, unit_size, maxint,tot_sam,...
	usewaitbar, waitbarmessage)
% READVHLVDATAFILE_MULTIPLEXED - faster code that reads N channels of multiplex data
%
% INPUTS:
%    HEADERSTRUCT - The header structure
%    CHANNEL - The channel number, from 1, where 1 is the first channel recorded
%    T0 - the time value to start on
%    T1 - the time value to end on

s0 = round(1+t0*headerstruct.SamplingRate); % samples run from 1...N, sample 1 occurs at t==0
s1 = min(round(1+t1*headerstruct.SamplingRate),tot_sam); % make sure it does not exceed tot_sam
t = (s0-1:s1-1)/headerstruct.SamplingRate;

data = feval(output_precision,zeros(s1-s0+1,length(channels)));

if usewaitbar,
	h = waitbar(0,waitbarmessage);
else,
	h = [];
end;

for c=1:length(channels),
	fseek(fid,((s0-1)*headerstruct.NumChans+(channels(c)-1))*unit_size,'bof');
	if isfield(headerstruct,'Scale'),
		data(:,c) = feval(output_precision,(headerstruct.Scale/maxint)*fread(fid,(s1-s0+1),precision,unit_size*(headerstruct.NumChans-1),'ieee-be'));
	else,
		data(:,c) = fread(fid,(s1-s0+1),precision,unit_size*(headerstruct.NumChans-1),'ieee-be');
	end;
	if usewaitbar,
		waitbar(c/length(channels),h);
	end;
end;
