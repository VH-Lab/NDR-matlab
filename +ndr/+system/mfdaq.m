% NDR_SYSTEM_MFDAQ - Multifunction DAQ object class
%
% The ndr.system.mfdaq object class.
%
...
% 
%
% See also: ndr.system.mfdaq/ndr.system.mfdaq
%

classdef mfdaq < ndr.system
	...
	methods
		function obj = mfdaq(varargin)
			% ndr.system.mfdaq - Create a new multifunction DAQ object
			%
			%  D = ndr.system.mfdaq(NAME, THEFILENAVIGATOR)
			%
			%  Creates a new ndr.system.mfdaq object with NAME, and FILENAVIGATOR.
			%  This is an abstract class that is overridden by specific devices.
				obj = obj@ndr.system(varargin{:});

				if ~isempty(obj.daqreader),
					if ~isa(obj.daqreader,'ndi.daq.reader.mfdaq'),
						error(['The DAQREADER for an ndr.system.mfdaq object must be a type of ndr.reader.mfdaq.']);
					end;
				end;
		...
		function channels = getchannels(ndi_daqsystem_mfdaq_obj)
			...
			%
				...
				N = numepochs(ndr_system_mfdaq_obj);

				for n=1:N,
					epochfiles = getepochfiles(ndr_system_mfdaq_obj.filenavigator, n);
					channels_here = getchannelsepoch(ndr_system_mfdaq_obj.daqreader, epochfiles);
					channels = vlt.data.equnique( [channels(:); channels_here(:)] );
				end
		end; % getchannels

		function data = readchannels_epochsamples(ndr_system_mfdaq_obj, channeltype, channel, epoch, s0, s1)
		...
		function data = readchannels(ndr_system_mfdaq_obj, channeltype, channel, timeref_or_epoch, t0, t1)
			...
			if isa(timeref_or_epoch,'ndi.time.timereference'),
				...
				if epoch0_timeref.epoch~=epoch1_timeref.epoch,
					error(['Do not know how to read across epochs yet; request spanned ' ...
						...
						' and ' ndr_system_mfdaq_obj.filenavigator.epoch2str(epoch1_timeref.epoch) '.']);
				end
				epoch = epoch0;
			else,
				epoch = timeref_or_epoch;
			end
			sr = samplerate(ndr_system_mfdaq_obj, epoch, channeltype, channel);
			...
			s0 = 1+round(sr*t0);
			s1 = 1+round(sr*t1);
			[data] = readchannels_epochsamples(ndr_system_mfdaq_obj, epoch, channeltype, channel, s0, s1);
		end %readchannels()

		function data = readevents(ndr_system_mfdaq_obj, channeltype, channel, timeref_or_epoch, t0, t1)
			...
			if isa(timeref_or_epoch,'ndi.time.timereference'),
				...
				[data] = readevents_epochsamples(ndr_system_mfdaq_obj,channeltype,channel,epoch,t0,t1);
			end
		end % readevents

		function [data, timeref] = readevents_epochsamples(ndr_system_mfdaq_obj, channeltype, channel, epoch, t0, t1)
        ...
        function sr = samplerate(ndr_system_mfdaq_obj, epoch, channeltype, channel)
			...
			% that CHANNELTYPE applies to every entry of CHANNEL.
				...
				epochfiles = getepochfiles(ndr_system_mfdaq_obj.filenavigator, epoch);
				sr = ndr_system_mfdaq_obj.daqreader.samplerate(epochfiles, channeltype, channel); 
		end;
