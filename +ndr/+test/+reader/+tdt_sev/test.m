...
% ndr.test.reader.tdt_sev.test - Test reading using Neuroscience Data Reader with TDT SEV format
%
%  See also: ndr.reader.tdt_sev
%
	plotit = 1;
	...
	r = ndr.reader('sev'); % Open a .SEV reader
	...
	if plotit,
		...
		xlabel('Time (s)');
		ylabel('Data values');
		title(['SEV example data']);
    end;
    
end % ndr.test.reader.tdt_sev.test
