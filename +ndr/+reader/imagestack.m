...
% Video, ...).
%
% NANSEN is an OPTIONAL dependency. It is loaded only when this reader is
% used; it is never required by NDR's native readers (e.g. ndr.reader.tiffstack)
% and never reaches NDI. If NANSEN is not on the MATLAB path, the methods of
% this class raise a clear error.
%
...
classdef imagestack < ndr.reader.base
	...
	methods
		...
		function s = imagestackobject(imagestack_obj, epochstreams)
			...
			%
			% Builds a nansen.stack.ImageStack for the epoch. Errors with a
			% clear message if NANSEN is not installed.
			%
				if exist('nansen.stack.ImageStack','class')~=8
					error('ndr:reader:imagestack:nonansen',...
						['nansen.stack.ImageStack was not found on the MATLAB path. ' ...
						 ...
						 '(https://github.com/VervaekeLab/NANSEN). Use ndr.reader.tiffstack ' ...
						 'for plain multipage TIFFs to avoid this dependency.']);
				end
				filename = imagestack_obj.filenamefromepochfiles(epochstreams);
				s = nansen.stack.ImageStack(filename);
		...
		function frames = readframes(imagestack_obj, epochstreams, epoch_select, frameind)
			...
			%
			% Adapted from nansen.stack.ImageStack/getFrameSet. Returns frames
			% in 'YXCZT' order.
				if nargin<4
					frameind = 1:imagestack_obj.numframes(epochstreams, epoch_select);
				end
