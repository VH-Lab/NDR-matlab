...
The **values** come out through `obsColumns`, by name -- naming a categorical
column there returns its decoded category names, one per cell, in the same call
as any numeric columns. That is deliberately not the default: the default is
measurements, and a labeling is a claim about each cell rather than a
measurement of it, so which one to believe stays the caller's decision.

pandas writes code `-1` for a cell the labeling never assigned, and both ports
turn that into `''` -- the empty string `makeCellTypeLabels` documents as
UNLABELED. It is a branch rather than a fallback: in Python, numpy's negative
indexing would make `-1` address the **last** category, giving those cells a
real, plausible label that nothing downstream could catch. `cellbin_unlabeled.h5ad`
exists only to reach that byte, which cannot be written by naming a category.
