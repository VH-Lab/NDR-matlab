classdef readOmeZarr < matlab.unittest.TestCase
    ...
    methods (Test)
        function testReadOmeZarrArtifacts(testCase)
            ...
            regions = repmat(struct( ...
                'name', '', 'pyramid', '', 'level', 0, ...
                'region', [], 'sha256', ''), 1, numel(regionSpecs));
            for k = 1:numel(regionSpecs)
                spec = regionSpecs{k};
                regions(k) = makeRegionEntry(testCase, fixtureDir, ...
...
function entry = makeRegionEntry(testCase, fixtureDir, name, pyramid, level, region)
    if isempty(region)
