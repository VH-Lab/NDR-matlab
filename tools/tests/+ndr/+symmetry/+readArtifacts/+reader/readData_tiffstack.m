classdef readData_tiffstack < matlab.unittest.TestCase
    % READDATA_TIFFSTACK - Read back and verify symmetry artifacts for the NDR
    % tiffstack reader (mirrors readData.m for the intan reader).
    %
    ...
    methods (Test)
        function testReadDataTiffstackArtifacts(testCase, SourceType)
            ...
            testCase.assumeTrue(isfile(tiff_file), ...
                'Example TIFF file not available; skipping readArtifacts test.');
